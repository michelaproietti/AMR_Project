...
Eigen::VectorXd ChooseParent(vector<Eigen::VectorXd> ZNear, Eigen::VectorXd zNearest, Eigen::Vector3d zNew, vector<Eigen::Vector3d> plan) {
	...
	for(int i = 0; i < ZNear.size(); i++) {
		...
		//cout << "PLAN SIZE choose_parent ======= " << plan.size() << endl;
		if (plan.size()==0) { //plan.empty() || 
			cout << "_______________________________________________EXCEPTION AVOIDED choose_parent " << endl;
			continue;
			//cout << "ITERATION_____ choose_parent : " << i << endl;
		}
...
void rewire(vector<Eigen::VectorXd> ZNear, Eigen::VectorXd zMin, Eigen::VectorXd zNew){
	for(int i = 0; i < ZNear.size(); i++) {
		...
		//cout << "PLAN SIZE rewire ======= " << plan.size() << endl;
		if (plan.size()==0) { //plan.empty() || 
			cout << "_______________________________________________EXCEPTION AVOIDED rewire " << endl;
			continue;
			cout << "ITERATION_____rewire : " << i << endl;
		}
		
		
		if(check_collisions(plan) && zNew[5]+cost_plan(plan) < ZNear[i][5]) {   
			cout << "TREE REWIRED" << endl;
...
int is_in_tree2(Eigen::Vector3d qRand) {
	int tree_size = tree.size();
	
	for (int i=0; i<tree_size; i++) {
		//if (tree[i].z.head(3) == qRand) return i;
		if ( abs(tree[i].z[0] - qRand[0]) < 1e-4 && abs(tree[i].z[1] - qRand[1]) < 1e-4 && abs(tree[i].z[2] - qRand[2]) < 1e-4 ) return i;
	}
...
void RRTstar(Eigen::Vector3d q_i, Eigen::Vector3d q_f){  //returns the same type we defined for the tree
	...
	for(i = 0; i < maxIter; i++) {
		...
		if (check_collisions(plan)){
			...
			if (dist_x <= 0.25 && dist_y <= 0.25) update_goal_info(qNew, i, dist_x, dist_y, dist_theta, q_i, q_f, start);
		}
	}
